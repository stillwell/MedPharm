...
- **Shared PostgreSQL backend (multi-client).** Every server-side component — the PyQt6 desktop, Flask web portal, Cloud REST API, Docker, and Kubernetes — can now attach to a **single shared database** by setting `MEDPHARM_DATABASE_URL` (e.g. `postgresql+psycopg://user:pass@host:5432/medpharm`). `DatabaseManager` builds its engine from any SQLAlchemy URL; left unset, each component keeps its private single-file SQLite database exactly as before. The Docker Compose stacks now ship a `db` (PostgreSQL 16) service and the Kubernetes deployment runs a `medpharm-postgres` StatefulSet — so the server is no longer pinned to one replica. Native clients (Android/iOS/macOS/Windows) are unchanged: they reach the shared data through the REST API, never the database directly. See [Environment Variables](#environment-variables) and [`database/README.md`](database/README.md).
...
└────────┬────────────┘         │         └──────────┬───────────┘
   ...
   │ single file,     │  │ networked — desktop, │
   │ one process      │  │ web, API, Docker &   │
   │                  │  │ k8s all intermingle  │
   └──────────────────┘  └──────────────────────┘
        23 Models · 75+ Medications · 30+ Symptoms ·
        25+ Conditions · 24 Interactions
```
...
5. Initialize the database (SQLite by default, or a shared PostgreSQL when `MEDPHARM_DATABASE_URL` is set) and seed it with sample data
...
- DB backup uses `sqlite3 .backup` (online, consistent) when available; falls back to `cp` of the file plus any WAL/SHM sidecars. This covers **SQLite only** — a shared `MEDPHARM_DATABASE_URL` PostgreSQL deployment should be backed up with `pg_dump` (the file-based auto-backup does not apply to it).
- When using SQLite, the DB path is auto-detected in this priority order: `$MEDPHARM_DB_PATH` env var → `medpharm_erp.db` (project root, the `run_cloud.py` default) → `data/medpharm_erp.db` (Docker volume default) → `data/medpharm.db` (legacy). When `MEDPHARM_DATABASE_URL` is set, path auto-detection is skipped.
...
|--------|-------------|---------|
| `medpharm-pgdata` | `/var/lib/postgresql/data` | PostgreSQL data for the shared `db` service (default backend) |
| `medpharm-data` | `/data` | SQLite database (`medpharm_erp.db`) — used only when `MEDPHARM_DATABASE_URL` is unset |
...
| Qt desktop (clinical staff) | Talks to the database directly through `DatabaseManager`, not the REST API, so the ngrok URL doesn't apply. To share the server's data, point it at the same database with `MEDPHARM_DATABASE_URL` (e.g. PostgreSQL); otherwise it uses a local SQLite file. |
...
| `MEDPHARM_DB_PASSWORD` | `change-this-in-production` | Password for the Docker Compose `db` (PostgreSQL 16) service and the `MEDPHARM_DATABASE_URL` it builds |
