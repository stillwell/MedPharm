...
./k8s/medpharm-k8s.sh deploy --tag=1.7.5                     # rolling upgrade
...
docker pull enlightec/medpharm-api:1.7.5
```
...
```bash
git tag v1.7.5
git push origin v1.7.5   # Triggers the pipeline
```
...
| `1.7.5`, `1.7.4`, `1.7.3`, `1.7.2`, `1.6.0`, `1.1.1`, `1.1.0`, `1.0.0` | Pinned semantic version tags (published from `v*.*.*` git tags) |
...
./install.sh --docker --tag=1.7.5         # Pin to a specific release
```
...
> **Note:** Earlier 1.7.x builds emitted a Volume III (Developer Manual) whose cover-page navy backdrop bled into every body page, rendering long stretches of text as dark-blue text on a dark-blue field, and inserted a blank page before each part divider. Both defects were corrected in 1.7.5; if you have an older locally-built copy, regenerate it with `python3 docs/generate_developer_manual.py` to pick up the fix.
