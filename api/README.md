...
| `auth.py` | JWT encode/decode (HMAC-SHA256), `@require_auth`, `@require_role` decorators |
...
2. Decorate it with `@require_auth` (any user) or `@require_role('doctor','admin')` as appropriate.
3. Use `DatabaseManager` (from `database/db_manager.py`) for all data access — never hit SQLAlchemy directly inside a handler.
